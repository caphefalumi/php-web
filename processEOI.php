...
$stmt = $conn->prepare("INSERT INTO EOI (jobRef, firstName, lastName, dob, gender, address, town, state, postcode, email, phone, skills, otherSkills) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("sssssssssssss", $jobRef, $firstName, $lastName, $dob, $gender, $address, $town, $state, $postcode, $email, $phone, $skills, $otherSkills);

if ($stmt->execute()) {
    echo "<h3>Thank you for your submission!</h3>";
    echo "<p>Your EOI number is: " . $stmt->insert_id . "</p>";
} else {
